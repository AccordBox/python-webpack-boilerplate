...
def npm_project_path(tmp_path, request):
    ...
    cookiecutter(
        ...
        no_input=True, output_dir=str(frontend_project_root)
    )
    ...
    result = subprocess.run(
        command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, cwd=str(frontend_project_path),)
